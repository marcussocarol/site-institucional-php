<?php
/**
 * SERVIÇO DE VALIDAÇÃO COM IA
 * Responsável apenas por conversar com o Google Gemini.
 */
function validarEmailComIA($email) {
    // -------------------------------------------------------------------------
    // PASSO 1: CONFIGURAÇÃO (COLOQUE SUA CHAVE AQUI DEPOIS DE EXTRAIR)
    // -------------------------------------------------------------------------
    $apiKey = 'AIzaSyD4yRvQAnnAsGtLKxalC807CKjOrXxMLWE'; // <--- SUA CHAVE API VAI AQUI ENTRE AS ASPAS

    // Fail-open: Se não tiver chave, aprova para não travar o site em apresentação
    if (empty($apiKey)) {
        return true; 
    }

    // URL da API (Modelo Flash 1.5 - Rápido e Gratuito)
    ...
    // Se a IA disse INVALIDO, retornamos false (bloqueia o cadastro)
